...
class MPA_Lya(torch.autograd.Function):
    @staticmethod
    def forward(ctx, M):
        norm_m = torch.norm(M, dim=[1, 2]).reshape(M.size(0), 1, 1)
        ...
        M_sqrt = matrix_pade_approximant(M / norm_m, I)
        M_sqrt = M_sqrt * torch.sqrt(norm_m)
        ctx.save_for_backward(M, M_sqrt, norm_m, I)
        return M_sqrt
